        ...
        {isSecondary && s.dest_ns != null && (
          <div className="sp-section">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(86px, 1fr))', gap: 8, marginBottom: 10 }}>
              {[['Education', s.dest_edu, '#1d5a9e'], ['Apprenticeship', s.dest_app, '#0d7a42'], ['Employment', s.dest_work, '#5b3fa0'], ['No sustained dest.', s.dest_ns, '#b91c4a']].map(([l, v, c]) => (
                <div key={l} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '8px 10px' }}>
                  <div style={{ fontSize: '1.15rem', fontWeight: 800, color: c }}>{v == null ? '–' : v + '%'}</div>
                  <div style={{ fontSize: '0.66rem', color: '#64748b', fontWeight: 600, lineHeight: 1.2 }}>{l}</div>
                </div>
              ))}
            </div>
            {(s.dest_ns_dis != null || s.dest_adm) && (
              <div style={{ fontSize: '0.76rem', color: '#475569', lineHeight: 1.5 }}>
                {s.dest_adm && s.dest_adm !== 'Not collected' && <span><b>{s.dest_adm}</b> admissions. </span>}
                {s.dest_ns_dis != null && <span>No sustained destination: <b style={{ color: '#b91c4a' }}>{s.dest_ns_dis}%</b> of disadvantaged pupils{s.dest_ns_nondis != null ? <span> vs {s.dest_ns_nondis}% of others</span> : null}.</span>}
              </div>
            )}
          </div>
        )}
